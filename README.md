...
> 真实支付网关和 webhook 尚未接入；生产构建会禁用模拟支付与退款，并把购买、付款和退款需求接入可追踪的人工咨询流程。开发环境仍保留 mock 流程供联调。
...
- 验证结果：API 36 suites / 338 tests；Web 22 files / 132 tests；浏览器 E2E 29 passed / 1 skipped；生产构建和 TypeScript 检查通过
...
开发环境订单支付仍是 mock，生产环境接口会拒绝模拟支付，UI 会把购买、付款和退款需求带着课程或订单上下文转入企业咨询表单。不要仅通过设置支付密钥就把它视为生产支付。正式开放自动支付前需要：
