...
| 指标 | 数字 |
| --- | --- |
| API 测试 | **42 suites / 363 tests** 全过 |
| Web 测试 | **30 files / 157 tests** 全过 |
| E2E (Playwright) | **43 passed / 3 conditional skips**（桌面/移动互斥场景均有对应覆盖） |
...
| Go API 迁移 | **实验性迁移实现，禁止生产切流**：仍有 AI/URL 导入占位实现、生产上传未接通，完整 Docker E2E 尚未形成稳定门禁 |
...
flowchart LR
  ...
  subgraph API["API (apps/api, NestJS 11)"]
    ...
    HEALTH["/api/v1/health + /api/v1/health/ready<br/>(redis / mysql / minio 三件套)"]
  end
...
- **可观测**:`/api/v1/health/ready` 暴露 mysql/redis/minio 三件套状态
- **渐进式 Go 迁移**:`apps/api-go` 仅用于迁移验证；NestJS 是唯一生产 API。AI、URL 导入、对象存储和完整 E2E 未通过切流门禁前，禁止将流量路由到 Go 实现。
...
│   │
│   ├── api-go/                             Go/Fiber 实验性迁移实现（禁止生产切流）
...
| **health** | `GET /api/v1/health` (liveness) / `GET /api/v1/health/ready` (mysql+redis+minio) | 公开 |
...
| 维度 | 当前 | 触发条件 |
| --- | --- | --- |
| API Jest | 42 suites / 363 tests 全过 | `apps/api` 改 controller / service / dto |
| Web Vitest | 30 files / 157 tests 全过 | `apps/web` 改组件 / hook / lib |
| E2E Playwright | 43 passed / 3 conditional skips | 任何路由 / 鉴权 / 公开页改动 |
