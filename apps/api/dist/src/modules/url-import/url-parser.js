"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.parseVideoUrl = parseVideoUrl;
const common_1 = require("@nestjs/common");
const YOUTUBE_HOSTS = new Set([
    'youtube.com',
    'www.youtube.com',
    'm.youtube.com',
    'youtu.be',
]);
const BILIBILI_HOSTS = new Set([
    'bilibili.com',
    'www.bilibili.com',
    'm.bilibili.com',
    'b23.tv',
]);
function parseVideoUrl(rawUrl) {
    let url;
    try {
        url = new URL(rawUrl.trim());
    }
    catch {
        throw new common_1.BadRequestException('Invalid URL');
    }
    if (url.protocol !== 'https:' && url.protocol !== 'http:') {
        throw new common_1.BadRequestException('Only http/https URLs are supported');
    }
    const host = url.hostname.toLowerCase();
    if (YOUTUBE_HOSTS.has(host)) {
        return parseYouTube(url);
    }
    if (BILIBILI_HOSTS.has(host)) {
        return parseBilibili(url);
    }
    throw new common_1.BadRequestException(`Unsupported platform: ${host}. Supported: youtube.com, youtu.be, bilibili.com`);
}
function parseYouTube(url) {
    let videoId = null;
    if (url.hostname === 'youtu.be') {
        videoId = url.pathname.replace(/^\//, '').split('/')[0] || null;
    }
    else if (url.pathname.startsWith('/watch')) {
        videoId = url.searchParams.get('v');
    }
    else if (url.pathname.startsWith('/embed/')) {
        videoId = url.pathname.split('/')[2] || null;
    }
    else if (url.pathname.startsWith('/shorts/')) {
        videoId = url.pathname.split('/')[2] || null;
    }
    if (!videoId || !/^[A-Za-z0-9_-]{6,15}$/.test(videoId)) {
        throw new common_1.BadRequestException('Could not extract YouTube video ID from URL');
    }
    return {
        platform: 'youtube',
        videoId,
        canonicalUrl: `https://www.youtube.com/watch?v=${videoId}`,
        embedUrl: `https://www.youtube.com/embed/${videoId}`,
    };
}
function parseBilibili(url) {
    const match = url.pathname.match(/\/video\/(BV[A-Za-z0-9]+|av\d+)/i);
    if (!match) {
        throw new common_1.BadRequestException('Could not extract Bilibili video ID from URL');
    }
    const id = match[1];
    const isBV = id.toLowerCase().startsWith('bv');
    return {
        platform: 'bilibili',
        videoId: id,
        canonicalUrl: `https://www.bilibili.com/video/${id}`,
        embedUrl: `https://player.bilibili.com/player.html?bvid=${id}&autoplay=0`,
    };
}
//# sourceMappingURL=url-parser.js.map