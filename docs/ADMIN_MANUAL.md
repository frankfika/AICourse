...
- **基本信息**:邮箱 / 昵称 / 头像 / 角色
- **学习概况**:已报名课程 / 完成课程 / 学习中 / 总学习时长
- **订单**:最近 10 笔订单
- **证书**:已颁发证书列表
- **积分**:当前积分 + 历史流水
- **活动日志**:最近 30 天登录 / 报名 / 完成课程

### 6.3 修改角色

抽屉内「角色」下拉,仅 super_admin 可见。可选:
- `user`(普通)
- `admin`(运营)
- `super_admin`(技术负责人)

切换会**立即踢出该用户的会话**(refresh token 失效)。
...
抽屉内「授权课程」:
- 输入课程 ID(逗号分隔,批量)
- 选过期时间(永久 / 7 天 / 30 天 / 自定义)
- 「授权」→ 写入 enrollments 表 → 用户立即可学
