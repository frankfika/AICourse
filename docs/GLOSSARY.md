...
### **Module / 模块**
课程内的内容组织单元。一个课程可以只有一个模块，也可以包含多个模块；每个模块下面包含一个或多个课时。数据库和接口仍使用历史字段 `chapter`，但面向用户统一称为“模块”，避免与 Nano Degree 层级混淆。

### **Lesson / 课时**
课程中可独立学习的一节内容，通常对应一个视频，也可以是文档、练习或其他学习材料。课程的学习量和 Nano Degree 的总学习量统一按“课时”统计。

### **Syllabus / 教学大纲**
课程整体规划(目标 / 模块列表 / 时长 / 先修要求),通常在课程详情页顶部可见。
