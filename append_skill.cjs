...
if (existing.indexOf('Part 3') === -1) {
    // maybe it was not appended or it was corrupted before
    const lastValid = existing.lastIndexOf('## Resources');
    if (lastValid !== -1) {
        existing = existing.substring(0, existing.indexOf('\n', lastValid) + 1);
        // keep up to the end of Resources section
    }
}
